...
Button::Button( Scene* parent, Model* model, ModelType type, glm::vec3* vtx, glm::vec2* textureCoordinates, char* texture ):Model(parent, model, type)
{
    ...
    char fname[MAX_PATH]= {""};
    strcpy( fname, "/data/local/tmp/Images/" );
...
{
    LOGI("dlgmlals3 TouchEventDown    %f %f\n", x, y);
    GLint viewport_matrix[4]	= { 0, 0, 1340, 2770 };
    glGetIntegerv( GL_VIEWPORT, viewport_matrix );
    LOGI("dlgmlals3 TouchEventDown  %d %d %d %d\n",
         viewport_matrix[0], viewport_matrix[1], viewport_matrix[2], viewport_matrix[3]);
    ...
    glm::vec3 nearPoint = glm::unProject(win, *TransformObj->TransformGetModelViewMatrix(),
                                         *TransformObj->TransformGetProjectionMatrix(), viewport);
    win.z = 1.0;
    glm::vec3 farPoint = glm::unProject(win, *TransformObj->TransformGetModelViewMatrix(),
                                        *TransformObj->TransformGetProjectionMatrix(), viewport);
    Ray ray0(nearPoint, farPoint-nearPoint);
    glm::vec3 intersectionPoint;
    
    if(IntersectWithRay( ray0, intersectionPoint)){
        LOGI("Intersect with %s", this->GetName().c_str());
        isPicked = !isPicked;
        clicked = true;
        return;
    }
...
bool Button::IntersectWithRay(Ray ray0, glm::vec3& intersectionPoint)
{
    ...
    if ( glm::intersectLineTriangle(ray0.destination, ray0.direction, vec3(p0.x,p0.y,p0.z),
                                    vec3(p1.x,p1.y,p1.z), vec3(p2.x,p2.y,p2.z), intersectionPoint)){
        return true;
    }
